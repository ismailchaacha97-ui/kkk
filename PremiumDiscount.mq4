...
//|                                                                   |
//|  Anchor modes: 0 = previous daily candle (stable),                |
//|                1 = current developing daily candle,               |
//|                2 = most recent completed Asia session.            |
//|                                                                   |
...
//| Inputs                                                           |
//+------------------------------------------------------------------+
//--- 1. Anchor / dealing range
input int    InpAnchorMode       = 0;                     // Anchor mode: 0=Previous day | 1=Current day | 2=Asia session
...
input int    InpFillAlpha        = 45;                    // Fill opacity 0-255 (MT5 only; MT4 draws pale fill)
...
//--- 3. Alerts (bar close only - never repaints)
...
bool UpdateSession()
  {
   ...
   if(!s.valid)
     {
      int k0 = (InpAnchorMode == 1) ? 0 : 1;  // CUR_DAY starts at today's D1 bar
...
bool EnsureObj(string id, int type)
  {
   string name = O(id);
   if(ObjectFind(name) < 0)
      return ObjectCreate(name, (ENUM_OBJECT)type, 0, 0, 0, 0, 0);
   return true;
  }

void SetRect(string id, datetime t0, double p0, datetime t1, double p1,
             color clr, bool visible)
  {
...
void DeleteKnownObjects()
  {
   string ids[] = {"BAND_PREM","BAND_DISC","DEEP_PREM","DEEP_DISC",
                   "L_TOP","T_TOP","L_BOTTOM","T_BOTTOM","L_EQ","T_EQ",
                   "L_DEEPP","T_DEEPP","L_DEEPD","T_DEEPD",
                   "L_HALFP","T_HALFP","L_HALFD","T_HALFD",
                   "L_EXTP","T_EXTP","L_EXTD","T_EXTD","PRICE",
                   "PNL_MAIN","PNL_ZONE","PNL_REST"};
...
void DrawSession()
  {
   ...
   //--- zone fills (drawn behind the candles; MT4 has no alpha -> pale colors)
   if(InpShowZoneFill)
     {
      SetRect("BAND_PREM", t0, lv.eq, t1, lv.top, InpColorFillPremium, true);
      SetRect("BAND_DISC", t0, lv.bot, t1, lv.eq, InpColorFillDiscount, true);
      if(InpShowDeepFill)
        {
         SetRect("DEEP_PREM", t0, lv.deepPrem, t1, lv.top, InpColorFillDeep, true);
         SetRect("DEEP_DISC", t0, lv.bot, t1, lv.deepDisc, InpColorFillDeep, true);
        }
      else
        {
         ShowObj("DEEP_PREM", false);
...
int OnInit()
  {
   ...
   DeleteKnownObjects();               // clean leftovers of this chart instance
   g_lastBarTime = 0;
   g_sess.valid = false;
   ResetSessionState();
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   //--- MT4 arrays are series: index 0 = current bar
