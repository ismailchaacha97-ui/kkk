...
// --- Plot 6: Bullish Liquidity Sweep & Absorption ---
#property indicator_label6  "Bullish Liquidity Sweep"
#property indicator_type6   DRAW_ARROW
#property indicator_color6  clrAqua
#property indicator_style6  STYLE_SOLID
#property indicator_width6  1

// --- Plot 7: Bearish Liquidity Sweep & Absorption ---
#property indicator_label7  "Bearish Liquidity Sweep"
...
enum ENUM_GEX_MODE
{
   GEX_MODE_AUTO = 0,    // Auto-Calculated Dynamic Volatility & GEX Model
                ...
                const int &spread[])
{
   ...
   // 3. PERSISTENT ASYMMETRIC TARGET PLAN
   if(InpShowPersistentPlan && g_LastSetup.valid)
   {
      DrawMasterTradePlan(g_LastSetup);
   }
...
//+------------------------------------------------------------------+
//| Draw Master Trade Plan (Entry, SL, BE, TP1, TP2, TP3)            |
//+------------------------------------------------------------------+
void DrawMasterTradePlan(const TradeSetup &setup)
{
   ...
   DrawRayLine(PREFIX + "Plan_BE", startTime, setup.be, endTime, setup.be, clrWhite, STYLE_DOT, 1, StringFormat("Risk-Free BE (+%.1fR -> Move SL to Entry)", InpBreakevenTrigger_R));

   // 4. TP1 (+3R Target - 50% Banked)
   DrawRayLine(PREFIX + "Plan_TP1", startTime, setup.tp1, endTime, setup.tp1, clrMediumSeaGreen, STYLE_DASH, 2, StringFormat("TP1 (+%.0fR - Bank 50%%)", InpTarget1_R));
   ...
   // 6. TP3 (+10R Target - 20% Runner Home-Run)
   DrawRayLine(PREFIX + "Plan_TP3", startTime, setup.tp3, endTime, setup.tp3, clrMagenta, STYLE_SOLID, 2, StringFormat("TP3 (+%.0fR - Runner 20%%)", InpTarget3_R));
}
...
void RenderDashboard(double currentPrice)
{
   ...
   string deltaStr = StringFormat("Bar Delta: %+.0f | CVD: %+.0f", curDelta, BufferCVD[0]);
   ...
   if(g_LastSetup.valid)
   {
      ...
      string beStatus = (curR >= InpBreakevenTrigger_R) ? "[LOCKED ZERO-RISK] SL at Breakeven"
      ...
      string tp1Str = StringFormat("TP1 (+%.0fR - 50%%): %.2f (+$%.0f)", InpTarget1_R, g_LastSetup.tp1, g_LastSetup.riskDollars * InpTarget1_R);
